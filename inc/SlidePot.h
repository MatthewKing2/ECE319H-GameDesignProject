...
class SlidePot{ 
  private: // Data 
    ...
    int32_t flag;       // 0 if data is not valid, 1 if valid (semaphore)
    uint32_t distance;  // distance in 0.001cm, distance = (slope*data+offset)/4096
    uint32_t slope;     // calibration coeffients, "m"
    uint32_t offset;    // "b" 

  public: // Functions (in the order they would be called)
    SlidePot(uint32_t m, uint32_t b); // initialize slide pot (constructor w/ arguments)
